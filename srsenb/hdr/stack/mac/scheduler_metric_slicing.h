/*
 * TODO: Modified version of the DL_METRIC file provided by srsLTE
 *
...
class dl_metric_slicing : public sched::metric_dl
{
...
private:
  ...
  bool          find_allocation(uint32_t nof_rbg, rbgmask_t* rbgmask, uint16_t rnti);
  dl_harq_proc* allocate_user(sched_ue* user);

  srslte::log*           log_h     = nullptr;
  sched::dl_tti_sched_t* tti_alloc = nullptr;

  // TODO: Slicing related information
  std::map<int, rbgmask_t> slices;
  std::map<uint16_t, int> assigned_slice;

};

class ul_metric_slicing : public sched::metric_ul
{
public:
  void set_log(srslte::log* log_) final;
